...
**fhirlint** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `fhirlint` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
fhirlint ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
fhirlint ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
fhirlint ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
fhirlint ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `fhirlint` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="fhirlint") for rec in records]   # records = your JSON output

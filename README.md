...
[![PyPI](https://img.shields.io/pypi/v/cognis-fhirlint.svg)](https://pypi.org/project/cognis-fhirlint/)
[![CI](https://github.com/cognis-digital/fhirlint/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/fhirlint/actions)
...
FHIRLINT exists for one job — validate fhir r4/r5 resources and bundles against profiles (us core, etc.) with precise, line-level error reporting. — and does it without a SaaS bill or heavyweight setup.
...
```bash
fhirlint --version
fhirlint scan .                      # scan the current project
fhirlint scan . --format json
fhirlint scan . --fail-on high       # non-zero exit for CI gates
fhirlint mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **HL7 FHIR Validator + ESLint**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`fhirlint` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
